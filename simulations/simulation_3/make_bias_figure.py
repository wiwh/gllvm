"""Loading bias diagnostic for simulation 3 (Binomial N=10).

Same sign-corrected bias plot as simulation 1:

    signed_resid = sign(W_true) × (W_est − W_true)

vs loading magnitude |W_true|.  Negative = shrinkage toward zero, positive =
expansion.  Conditioning on sign(W_true) avoids cancellation between the two
sign groups in a naive mean.

Writes ``paper/figures/sim3_bias.pdf``.
"""
import os
import sys
...
def main():
    ...
    fig.suptitle(r"Binomial: sign-corrected loading bias vs. $|w_{ij}|$ (pooled over reps and $p$):"
